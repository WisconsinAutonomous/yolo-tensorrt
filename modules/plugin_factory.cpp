...
void YoloLayer::configureWithFormat(const Dims* inputDims,
                                    int nbInputs,
...
YoloLayer::YoloLayer(const uint32_t& numBoxes,
...
int YoloLayer::enqueue(int batchSize,
    ...
    NV_CUDA_CHECK(cudaYoloLayerV3(inputs[0], outputs[0], batchSize, _n_grid_h, _n_grid_w, m_NumClasses, m_NumBoxes,
                                  m_OutputSize, stream));
    return 0;
}
...
IPluginV2* YoloLayerPluginCreator::deserializePlugin(const char* name,
