...
//	m_TinyMaxpoolPaddingFormula(new YoloTinyMaxpoolPaddingFormula),
{
    ...
    m_EnginePath = networkInfo.data_path + "-" + m_Precision + "-batch" + std::to_string(m_BatchSize) + "-" +
                   std::to_string(m_InputH) + "x" + std::to_string(m_InputW) + "x" + std::to_string(m_InputC) +
                   ".engine";
...
void Yolo::createYOLOEngine(const nvinfer1::DataType dataType, Int8EntropyCalibrator* calibrator) {
        ...
        if (m_configBlocks.at(i).at("type") == "net") {
            ...
            if ((m_configBlocks.at(i).find("batch_normalize") != m_configBlocks.at(i).end()) &&
                ...
                layerType = "conv-bn-leaky";
                       ...
                       ("mish" == activation)) {
                out = net_conv_bn_mish(i, m_configBlocks.at(i), weights, trtWeights, weightPtr, channels, previous,
                                       m_Network);
            ...
            m_OutputTensors.at(outputTensorCount).volume = curYoloTensor.grid_h * curYoloTensor.grid_w *
            ...
            nvinfer1::IPluginV2* yoloPlugin = new nvinfer1::YoloLayer(
                m_OutputTensors.at(outputTensorCount).numBBoxes, m_OutputTensors.at(outputTensorCount).numClasses,
                m_OutputTensors.at(outputTensorCount).grid_h, m_OutputTensors.at(outputTensorCount).grid_w);
    ...
    if (dataType == nvinfer1::DataType::kINT8) {
        ...
        //	config->setTacticSources(1U << static_cast<uint32_t>(TacticSource::kCUBLAS) | 1U <<
...
void Yolo::create_engine_yolov5(const nvinfer1::DataType dataType, Int8EntropyCalibrator* calibrator) {
    ...
    for (uint32_t i = 0; i < m_configBlocks.size(); ++i) {
        ...
        if ("net" == m_configBlocks.at(i).at("type")) {
            ...
            nvinfer1::ILayer* out = layer_focus(trtWeights, "model." + std::to_string(i - 1), model_wts, previous,
                                                filters, kernel_size, trtWeights, m_Network);
            previous = out->getOutput(0);
            assert(previous != nullptr);
            channels = getNumChannels(previous);
            std::string outputVol = dimsToString(previous->getDimensions());
            ...
            nvinfer1::ILayer* out = layer_conv_bn_act(trtWeights, "model." + std::to_string(i - 1), model_wts, previous,
            ...
            int n_depth = (number > 1) ? (std::max(int(round(_f_depth_multiple * number)), 1)) : number;
            std::string s_model_name = "model." + std::to_string(i - 1);
            auto out = layer_bottleneck_csp(trtWeights, s_model_name, model_wts, m_Network, previous, n_out_channel,
                                            n_depth, short_cut);
            previous = out->getOutput(0);
            assert(previous != nullptr);
            channels = getNumChannels(previous);
            ...
            nvinfer1::IConcatenationLayer* concat = m_Network->addConcatenation(concat_tensor, vec_from.size());
            ...
            for (size_t ind_from = 0; ind_from < vec_from.size(); ++ind_from) {
                ...
                auto conv = layer_conv(trtWeights, s_model_name + ".m." + std::to_string(ind_from), model_wts,
                                       tensorOutputs[from], m_Network, n_filters, 1, 1, true);
                ...
                curYoloTensor.stride_h = m_InputH / curYoloTensor.grid_h;
                curYoloTensor.stride_w = m_InputW / curYoloTensor.grid_w;
                m_OutputTensors.at(ind_from).volume = curYoloTensor.grid_h * curYoloTensor.grid_w *
                                                      (curYoloTensor.numBBoxes * (5 + curYoloTensor.numClasses));
                ...
                nvinfer1::IPluginV2 *yoloPlugin = creator->createPlugin(("detect" + std::to_string(ind_from)).c_str(),
                pluginData);*/
                nvinfer1::IPluginV2* yoloPlugin = new nvinfer1::Detect(
                    curYoloTensor.numBBoxes, curYoloTensor.numClasses, curYoloTensor.grid_h, curYoloTensor.grid_w);
                assert(yoloPlugin != nullptr);
...
void Yolo::parseConfigBlocks() {
    for (auto block : m_configBlocks) {
        if (block.at("type") == "net") {
            ...
            if ((m_NetworkType == "yolov3") || (m_NetworkType == "yolov3-tiny") || (m_NetworkType == "yolov4") ||
            ...
            outputTensor.volume =
                outputTensor.grid_h * outputTensor.grid_w * (outputTensor.numBBoxes * (5 + outputTensor.numClasses));
...
void Yolo::parse_cfg_blocks_v5(const std::vector<std::map<std::string, std::string>>& vec_block_) {
    ...
    for (const auto& block : vec_block_) {
        if ("net" == block.at("type")) {
            assert((block.find("height") != block.end()) && "Missing 'height' param in network cfg");
            assert((block.find("width") != block.end()) && "Missing 'width' param in network cfg");
            assert((block.find("nc") != block.end()) && "Missing 'nc' param in network cfg");
            assert((block.find("depth_multiple") != block.end()) && "Missing 'depth_multiple' param in network cfg");
            assert((block.find("width_multiple") != block.end()) && "Missing 'width_multiple' param in network cfg");
            assert((block.find("anchors") != block.end()) && "Missing 'anchors' param in network cfg");
            assert((block.find("channels") != block.end()) && "Missing 'channels' param in network cfg");
            ...
            assert((block.find("from") != block.end()) && "Missing 'from' param in network cfg");
